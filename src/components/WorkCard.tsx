...
export function WorkCard({
  ...
  return (
    <div className="flex gap-5 flex-col sm:flex-row items-center sm:items-stretch ">
      <img src={imgPath} className="w-[250px] h-[280px] rounded-lg" />
      <div className="text-center sm:text-left max-w-[350px] sm:max-w-[500px]">
        {link ? (
          <a href={link} target="_blank" className='hover:underline hover:text-white'>
            <span className="text-[25px] font-semibold about-body-title">
              {project}
            </span>
          </a>
        ) : (
        ...
        <p className="mt-2 mb-3">
          <span className="second-title text-[18px] font-semibold">
            {description}
          </span>
        </p>
